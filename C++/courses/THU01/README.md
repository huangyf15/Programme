...
* 归纳
  ...
  * `protected` 成员的特点及意义
    ...
    * 对于其直接派生类来说（垂直访问），除了公有继承外，它与 `public` 成员的性质基本相同
...
### 运算符重载，`override` 与 `final`

### 虚函数和抽象类

* 初识虚函数
  * 用 `virtual` 关键字说明的函数，是实现运行时多态性基础
  * `C++` 中的虚函数是动态绑定的函数
...
* `virtual` 关键字
  * 派生类可以不显式地用 `virtual` 声明虚函数，这时系统就会用以下规则来判断派生类的一个函数成员是不是虚函数：
